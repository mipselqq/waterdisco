...
INSTALL_DIR="${INSTALL_DIR:-/opt/Waterdisco}"
...
require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
  die "Please run this installer as root (e.g. 'sudo ./script/install_linux_build.sh')"
  fi
}

usage() {
  cat <<'EOF'
Usage: ./script/install_linux_build.sh [options]

Options:
  --build-dir <path>   CMake build dir (default: ./build)
  --prefix <path>      Install dir (default: /opt/Waterdisco)
...
if [[ "$SKIP_BUILD" != "1" ]]; then
  ...
  log "[2/2] Building Waterdisco with nproc=${BUILD_JOBS} jobs (progress below)"
...
DESKTOP_DST="/usr/share/applications/waterdisco.desktop"
LOCAL_ICON_REL="Throne.png"
BIN_LINK_DST="/usr/local/bin/waterdisco"

log "Installing desktop entry"
cat > "$STAGE_DIR/waterdisco.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=Waterdisco
Comment=Waterdisco Proxy Utility
...
StartupNotify=true
EOF
install -m 0644 "$STAGE_DIR/waterdisco.desktop" "$DESKTOP_DST"

cat > "$STAGE_DIR/waterdisco" <<EOF
#!/usr/bin/env bash
exec "$INSTALL_DIR/Throne" -appdata "\$@"
EOF
install -m 0755 "$STAGE_DIR/waterdisco" "$BIN_LINK_DST"
